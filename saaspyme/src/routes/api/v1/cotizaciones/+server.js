...
function serializeCotizacion(cotizacion) {
	...
	return {
		...
		saldo,
		conceptos: cotizacion._count?.conceptos ?? cotizacion.conceptos?.length ?? 0,
		cliente: {
...
export async function GET(event) {
	...
	await rechazarCotizacionesVencidas(auth);

	const q = event.url.searchParams.get('q')?.trim() ?? '';
	const estado = event.url.searchParams.get('estado') ?? '';
	const sort = SORTS.has(event.url.searchParams.get('sort'))
		? event.url.searchParams.get('sort')
		: 'fecha';
	const dir = event.url.searchParams.get('dir') === 'asc' ? 'asc' : 'desc';
	const where = {
